...
async function parseExcelRows(matrix){
  ...
  // e, principalmente, como parte da chave única [REF|FILIAL|PC].
  // Não podemos trocar essa chave pela coluna PV real, pois isso faz o histórico de
  // finalizados deixar de casar e reabre programações antigas no painel.
  // A versão imediatamente anterior a esta chegou a usar PV real; abaixo limpamos,
  // somente para as linhas presentes neste arquivo, os IDs transitórios gerados por ela.
  const idsTransitóriosPvReal = new Set();
  const idCorretoPorIdPvReal = new Map();
  rows.forEach(r => {
    ...
    if(!ref || !filial || !pc || !pvReal) return;
    const idCorreto = [ref, filial, pc].join('|');
    const idPvReal = [ref, filial, pvReal].join('|');
    if(idPvReal !== idCorreto){
      idsTransitóriosPvReal.add(idPvReal);
      idCorretoPorIdPvReal.set(idPvReal, idCorreto);
    }
  });

  if(idsTransitóriosPvReal.size){
    state.finalizados = (state.finalizados || []).filter(r => !idsTransitóriosPvReal.has(r?.id));
    idsTransitóriosPvReal.forEach(id => state.remoteFinalizedIds?.delete(id));
    // Preserva uma eventual tratativa feita enquanto a versão transitória estava aberta.
    idsTransitóriosPvReal.forEach(idAntigo => {
      const idNovo = idCorretoPorIdPvReal.get(idAntigo);
      if(idNovo && state.tratativas?.[idAntigo] && !state.tratativas?.[idNovo]){
        state.tratativas[idNovo] = state.tratativas[idAntigo];
      }
      if(state.tratativas) delete state.tratativas[idAntigo];
    });
  }
  ...
  for(const r of rows){
    ...
    const filial = col(r, 'FILIAL_PC', 'FILIAL PC');
    // IMPORTANTE: manter PC aqui por compatibilidade com todo o histórico do painel.
    // Embora a tela chame esse campo de PV, desde a versão original ele lê a coluna PC.
    const pv = col(r, 'PC');
    const pvReal = col(r, 'PV');
    if(!ref || !pv) continue;
    validas++;
    ...
    const baseRow = {
      id, ref, dataPC, filial, pv, pvReal,
      cavalo: col(r, 'CAVALO'),
